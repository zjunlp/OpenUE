...
def IEDemo():
    ...
    for i,text in enumerate(text_list):
        ...
        resp = requests.post('http://'+config.tf_serving_addr+'/v1/models/class:predict', json=preprocess(text,'class'))
        end = time.time()
        print(resp.json())
        ...
        for item in predicate_predict:
            ...
            text2_raw = text + "\t" + item
            resp = requests.post('http://'+config.tf_serving_addr+'/v1/models/seq:predict', json=preprocess(text2,'seq'))
