...
import codecs
import numpy as np
from config_event import config
...
class SPO_Management():
    def __init__(self, text_sentence_list,token_in_not_UNK_list,token_label_list):
        self.relationship_label_list = config.class_label
        self.text_sentence_list = text_sentence_list
        self.token_in_not_UNK_list = token_in_not_UNK_list
        self.token_label_list = token_label_list

    def get_input_list(self,):
        [text_sentence_list, token_in_not_UNK_list, token_label_list] = self.text_sentence_list,self.token_in_not_UNK_list, self.token_label_list
        reference_spo_list = [None] * len(text_sentence_list)
        return text_sentence_list, token_in_not_UNK_list, token_label_list, reference_spo_list
    ...
    def produce_relationship_and_entity_sort_list(self):
        text_sentence_list, token_in_not_UNK_list, token_label_list, reference_spo_list = self.get_input_list()
        for [text_sentence, token_in_not_UNK, token_label, refer_spo_str] in\
                zip(text_sentence_list, token_in_not_UNK_list, token_label_list, reference_spo_list):
    ...
    def gen_triple(self,  keep_empty_spo_list=False):
        ...
        for text, text_predicate, entity_sort_tuple_list, refer_spo_list in self.produce_relationship_and_entity_sort_list():
            e_l = []
            res = {}
            for v,k in entity_sort_tuple_list:
                e_d = {}
                e_d['mention'] = k
                e_d['type'] = v
                e_d['predicate'] = text_predicate
                e_d['offset'] = text.find(k)
                e_l.append(e_d)
            res['text'] = text
            res['spo_list'] = e_l 
            return res
...
def IEDemo():
    ...
    if True:
        ...
        resp = requests.post('http://'+config.tf_serving_addr+'/v1/models/class_event:predict', json=preprocess(text,'class'))
        end = time.time()
        print(resp.json())
        ...
        for item in predicate_predict:
            ...
            resp = requests.post('http://'+config.tf_serving_addr+'/v1/models/seq_event:predict', json=preprocess(text2,'seq'))
            ...
            token_in_not_UNK_list.append(text2_not_UNK)
        ...
        spo_manager = SPO_Management(text_sentence_list,token_in_not_UNK_list,token_label_list)
        ...
        spo_list['text'] =  text
    return json.dumps(spo_list,ensure_ascii=False)
