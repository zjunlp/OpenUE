...
'''
def post_zeus(query,url):
    q_d = {}
    q_d['query'] = query
    return get_post(json.dumps(q_d),url)
...
def decode_openue_result(res,query):
    ...
    for k,v in res.items():
        ...
        for item in tags:
            t = item['object']
            if t.strip() == "." or t.strip() == "45th":
                continue
            r = item['predicate']
            h = item['subject']
            if h.strip() == "." or h.strip() == "45th":
                continue
            triple += '{subject:"' + h + '",     predicate:"' + r + '",     object:"' + t + '"},' 
            print(h+" "+r+" "+t+"</br>")
    triple = triple[:-1]
...
lan = check_lan(sentence)
if lan:
    url = "http://127.0.0.1:8888/IEDemo"
    res = post_zeus(sentence,url)
else:
    url = "http://127.0.0.1:8887/IEDemo"
    res = post_zeus(sentence,url)
