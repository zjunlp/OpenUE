...
class BertForNERHandler(BaseHandler, ABC):
    """
    ...
    def initialize(self, ctx):
        ...
        if self.setup_config["save_mode"] == "torchscript":
            ...
            # 载入推理模型
            self.model = BertForNER.from_pretrained(model_dir, config=config)
    ...
    def postprocess(self, data):
        ...
        需要将每一个batch 打包成为list, 现在未考虑多个requests情况
        """
        logger.info(f"postprocess: {data}")

        return [
            dict(
                outputs=dict(
                    predicate_probabilities=data[1].tolist(),
                    token_label_predictions=data[0].tolist()
                )
            )
        ]
...
if __name__ == "__main__":
    inference_model = BertForNERHandler()
