...
class Sequence_labeling_Processor(DataProcessor):
...
def main(_):
    ...
    processors = {
        "sequence_labeling": Sequence_labeling_Processor,
    }
        ...
        def serving_input_receiver_fn():
            ...
            features = {
                ...
                "segment_ids": tf.zeros(dtype=tf.int64,
                                        shape=[1, FLAGS.max_seq_length]),
                'predicate_label_id': tf.zeros(dtype=tf.int64, shape=[1]),
                'token_label_ids': tf.zeros(dtype=tf.int64, shape=[1, FLAGS.max_seq_length])
            }
