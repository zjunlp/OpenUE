...
class Trainer:
    def __init__(
        ...
        self.label_map_ner = label_map_ner
    ...
    def prediction_loop(
        ...
        for inputs in tqdm(dataloader, desc=description):
            loss, relation_output_sigmoid, label_id = self.prediction_step(model, inputs, prediction_loss_only)
            if loss is not None:
                eval_losses.append(loss)
            if relation_output_sigmoid is not None:
                # preds = logits if preds is None else torch.cat((preds, logits), dim=0)
                preds.extend(relation_output_sigmoid.tolist())
        ...
        if self.compute_metrics is not None and preds is not None and label_ids is not None:
            metrics = self.compute_metrics(predictions=preds, label_ids=label_ids)
        else:
            metrics = {}
        if len(eval_losses) > 0:
            metrics["predict_loss"] = np.mean(eval_losses)

        # Prefix all keys with eval_
        for key in list(metrics.keys()):
            if not key.startswith("predict_"):
                metrics[f"predict_{key}"] = metrics.pop(key)
    ...
    def prediction_step(
        ...
        labels = inputs.get("label_ids_seq")
